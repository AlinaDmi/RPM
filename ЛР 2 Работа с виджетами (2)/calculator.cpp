...
{
    QPalette p;
    p.setColor(backgroundRole(), QColor(170,70,70));
    this->setPalette(p);
    ...
    Button *squareRootButton = createButton(tr("Sqrt"), SLOT(multiplicativeOperatorClicked()));
...
bool Calculator::calculate(double rightOperand, const QString &pendingOperator)
{
    ...
    if (pendingOperator == tr("×")) {
        ...
        if (rightOperand == 0.0)
            return false;
        factorSoFar /= rightOperand;
    }else if (pendingOperator == tr("Sqrt")){
        if (factorSoFar < 0.0){
           abortOperation();
        }
        factorSoFar=pow(factorSoFar,1/rightOperand);
    }
